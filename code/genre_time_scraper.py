...
class Scraper(object):
    ...
    def _get_info(self, artist):
        url = self._get_info_url(artist)
        print url
        response = self._get_response('http://api.rovicorp.com/data/v1.1/name/info?apikey=pwj6rb7gm93xqece9bt5kqpk&sig=e093f90d59e2f1ea5742c93d4d7ff787&nameid=MN0000423829')
        print response
        ...
        if response == -1:
            ...
            genre_response = response['name']['musicGenres']['name']
            time_active_response = response['name']['active']

        print self.artist_ids[artist], 'is ', genre_response
        print self.artist_ids[artist], 'is ', time_active_response

        self.genre[self.artist_ids[artist]] = genre_response
        self.time[self.artist_ids[artist]] = time_active_response
    ...
    def _get_info_url(self, artist_id):
        return self.API_URL + 'info?apikey=%s&sig=%s&nameid=%s' % (self.KEY, self._get_sig(), artist_id)
...
def main():
    ...
    #scraper.genre = pickle.load(open('../data/genres.pickle', 'rb'))
    #scraper.time = pickle.load(open('../data/times.pickle', 'rb'))
    ...
    #pickle.dump(scraper.genre, open('genres.pickle', 'wb'))
    #pickle.dump(scraper.time, open('times.pickle', 'wb'))
